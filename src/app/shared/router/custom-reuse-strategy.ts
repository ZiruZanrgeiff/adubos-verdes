...
export class CustomReuseStrategy implements RouteReuseStrategy {
  ...
  constructor(strategyObservable: ReuseStrategyObservable) {
    ...
    // listening what paths need to be replaced
  ...
  shouldDetach(route: ActivatedRouteSnapshot): boolean {
    ...
    // this validation can be used to indicate which routes that needs to stored
    // let detach: boolean = route.data && (route.data as any).shouldDetach === true;
    return detach;
  }
  ...
  shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    // this will return false if the current route must be replaced
    for(let i = 0; i < this.shouldReplace.length; i++) {
      if(this.shouldReplace[i] === route.routeConfig.path){
        this.shouldReplace.splice(i);
        return false;
      }
    }
